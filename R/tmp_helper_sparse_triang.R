...
propagate_perfect_primes <- function(spt, perfect_lvs, perfect_par) UseMethod("propagate_perfect_primes")

propagate_perfect_primes.sparse_triang <- function(spt, rjt) {
  ...
  while (any(perfect_idx)) {
    ...
    for (k in seq_along(perfect_lvs)) {
      ...
      # TODO: Remove later
      if (is.null(message_k) || is.null(pot_par_k)) browser()
