...
while (any(spt$flawed)) {
  ...
  triangulate_root.sparse_triang <- function(spt) {
    ...
    rjt <- rooted_junction_tree(tmp_primes, spt$flawed_root_idx)$collect
  
    # inspection
    # par(mfrow = c(1, 2))
    # plot(mg)
    # igraph::graph_from_adjacency_matrix(rjt) %>% plot()
    
    propagate_perfect_primes.sparse_triang(spt, rjt)

    # calculate_flawed_root_msg <- function(spt, rjt) {
    # Find all "neiboring" nodes of the flawed prime
    flawed_nei_idx   <- which(rjt[, spt$flawed_root_idx] == 1L) # lvs[flawed_idx]
    ...
    # length(eg[[2]])
    # .map_int(cliques_mat_int_(eg2[["new_graph"]]), length) %>% max()
    ...
    # ----------
    # inspection
    # igraph::graph_from_adjacency_matrix(moral_graph, "undirected") %>% plot()
    # -----------
    ...
    if (neq_empt_int(spt$remaining_flawed_idx)) {
      ...
      # Now update cur_len :D !!?
      spt$cur_len <- spt$org_len
      spt$old_len <- spt$org_len # IS THIS CORRECT!?
    }
    ...
    # TODO: It goes wrong from here
    # .map_lgl(spt$tmp_potentials$prime_pots, is.null) %>% sum()
